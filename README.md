...
**Architecture Diagram:** https://link.excalidraw.com/readonly/GG79HoLOVxv4sSt79atc

**Presentation** https://docs.google.com/presentation/d/1ko9hb83aR7kAFITbFDHHh8sVIomOA-v8Z3-QDvLB4kM/edit?usp=sharing
...
2. **Crawling** — A concurrent crawler (4 threads) traverses the site, respecting max pages and max depth limits. It extracts page titles, descriptions, and content using Nokogiri. JavaScript-rendered pages are detected (Ferrum support is implemented but disabled in production). Crawl results are cached for 1 day.
...
4. **Output** — The grouped and enriched data is assembled into a Markdown file following the [llms.txt specification](https://llmstxt.org/). Results are written to the database and status updates are broadcast via Action Cable (WebSocket).

Supports Claude Sonnet 4.5 and GPT-5.2, or a no-model fallback that groups pages by URL path structure.

**Without API keys:** The app still works and uses path-based URL grouping instead of LLM grouping and enrichment.
...
Ensure you have all the prerequisites set up.
...
**Note:** Action Cable (Solid Cable) is used for live status updates. The cable database is set up automatically with `db:prepare`.
...
5. After deploying, run migrations: `heroku run bin/rails db:prepare`
